...
public partial class Search : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            this.BindataToRptSearch();
        }
    }

    private void BindataToRptSearch()
    {
        string sKeyword = Request.QueryString["keyword"].Trim().ToString();
       

        Products pc = new Products();
        rptSearch.DataSource = pc.GetByKeyword(sKeyword);
        rptSearch.DataBind();


        lblkeyword.Text = sKeyword;
        int nCount =  rptSearch.Items.Count;
        lblQuantity.Text = nCount.ToString();
    }
